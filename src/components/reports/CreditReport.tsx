...
const CreditReport: React.FC = () => {
  ...
  const loadCreditData = async () => {
    ...
    try {
      ...
      console.log('🔍 [CreditReport] Opening balances from database:', {
        ...
        .map(company => {
          ...
          if (companies?.indexOf(company) < 3) {
            console.log(`🔍 [CreditReport] Company "${company.company_name}" calculation:`, {
              ...
              creditBalance,
              formula: `${totalAdvancePayments} - ${openingBalance} - ${totalDues} = ${creditBalance}`
            });
          }
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Customer Name', 'Type', 'Opening Balance', 'Total Dues', 'Total Payments', 'Credit Balance', 'Phone', 'Email'];
    const csvContent = [
      headers.join(','),
