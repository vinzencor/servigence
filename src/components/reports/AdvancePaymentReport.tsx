...
interface AdvancePaymentData {
  transactions: AdvancePaymentTransaction[];
  summary: {
    totalAdvancePayments: number;
    transactionCount: number;
    averageAdvancePayment: number;
    largestAdvancePayment: number;
  };
...
const AdvancePaymentReport: React.FC = () => {
  const [data, setData] = useState<AdvancePaymentData | null>(null);
  ...
  const loadAdvancePaymentData = async () => {
    try {
      setLoading(true);
      const transactions = await dbHelpers.getAdvancePaymentTransactionsByDateRange(dateFrom, dateTo);
      
      if (!transactions) {
        setData({
          transactions: [],
          summary: { totalAdvancePayments: 0, transactionCount: 0, averageAdvancePayment: 0, largestAdvancePayment: 0 },
      ...
      // Transform data
      const advancePaymentTransactions: AdvancePaymentTransaction[] = transactions.map(t => ({
        id: t.id,
      ...
      // Calculate summary
      const totalAdvancePayments = advancePaymentTransactions.reduce((sum, t) => sum + t.amount, 0);
      const transactionCount = advancePaymentTransactions.length;
      const averageAdvancePayment = transactionCount > 0 ? totalAdvancePayments / transactionCount : 0;
      const largestAdvancePayment = Math.max(...advancePaymentTransactions.map(t => t.amount), 0);

      // Payment method breakdown
      const paymentMethodBreakdown = advancePaymentTransactions.reduce((acc, t) => {
      ...
      // Status breakdown
      const statusBreakdown = advancePaymentTransactions.reduce((acc, t) => {
      ...
      // Monthly trend
      const monthlyTrend = advancePaymentTransactions.reduce((acc, t) => {
      ...
      setData({
        transactions: advancePaymentTransactions,
        summary: { totalAdvancePayments, transactionCount, averageAdvancePayment, largestAdvancePayment },
        paymentMethodBreakdown,
        statusBreakdown,
        monthlyTrend
      });
    } catch (error) {
      console.error('Error loading advance payment data:', error);
      toast.error('Failed to load advance payment report data');
    } finally {
      setLoading(false);
    }
  };
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `advance-payment-report-${dateFrom}-to-${dateTo}.csv`;
    a.click();
    ...
    <div className="space-y-6">
        ...
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Advance Payment Report</h1>
                <p className="text-gray-500 mt-1">All advance payment transactions and prepayments</p>
              </div>
            </div>
            <div className="flex items-center space-x-3">
              <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500">Total Advance Payments</p>
              <p className="text-2xl font-bold text-blue-600">
                AED {data?.summary.totalAdvancePayments.toLocaleString('en-US', { minimumFractionDigits: 2 }) || '0.00'}
              </p>
            </div>
            <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{data?.summary.transactionCount || 0}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                AED {data?.summary.averageAdvancePayment.toLocaleString('en-US', { minimumFractionDigits: 2 }) || '0.00'}
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500">Largest Payment</p>
              <p className="text-2xl font-bold text-orange-600">
                AED {data?.summary.largestAdvancePayment.toLocaleString('en-US', { minimumFractionDigits: 2 }) || '0.00'}
              </p>
            </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold text-gray-900">Advance Payment Transactions</h2>
        ...
        {filteredTransactions.length === 0 && (
          <div className="text-center py-12">
            ...
            <h3 className="mt-2 text-sm font-medium text-gray-900">No advance payment transactions found</h3>
